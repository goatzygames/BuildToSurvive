...
void AShooterPlayerController::BeginPlay()
{
	...
	if (IsLocalPlayerController())
	{
		if (SVirtualJoystick::ShouldDisplayTouchInterface())
		{
			...
			{
				// add the controls to the player screen immediately
		...
		// delay spawning the shooter UI by 0.1 sec
		FTimerHandle TimerHandle;
		GetWorldTimerManager().SetTimer(
			TimerHandle,
			this,
			&AShooterPlayerController::SpawnShooterUI,
...
void AShooterPlayerController::SpawnShooterUI()
{
	...
	if (BulletCounterUIClass)
	{
		...
		if (BulletCounterUI)
		{
			BulletCounterUI->AddToViewport(999);
		}
		else
		{
			UE_LOG(LogBuildToSurvive, Error, TEXT("Could not spawn bullet counter widget."));
		}
	}
}
